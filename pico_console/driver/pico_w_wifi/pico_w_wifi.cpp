...
static int scan_result(void *env, const cyw43_ev_scan_result_t *result) {
  ...
  if (result) {
    LOG_PRINTF("ssid: %-32s rssi: %4d chan: %3d mac: %02x:%02x:%02x:%02x:%02x:%02x sec: %u\n",
  ...
  if (absolute_time_diff_us(get_absolute_time(), scan_time) < 0) {
    if (!scan_in_progress) {
      ...
      if (err == 0) {
        ...
        LOG_PRINTF("Failed to start scan: %d\n", err);
