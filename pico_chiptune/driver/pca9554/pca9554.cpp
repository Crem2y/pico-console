...
pca9554::pca9554(i2c_inst_t* i2c, int pin_scl, int pin_sda) {
  _i2c = i2c;
  _pin_scl = pin_scl;
  _pin_sda = pin_sda;
}
...
void pca9554::init(void) {
  i2c_init(_i2c, 1000 * 1000);          //Initialize I2C on i2c1 port with 1000kHz
...
void pca9554::get_btn_data(void) {
  ...
  i2c_write_blocking(_i2c, KEY_IC_A, buf, 1, true);
  i2c_read_blocking(_i2c, KEY_IC_A, buffer, 1, false); // [S1_U, S1_D, S1_L, S1_R, KEY_U, KEY_D, KEY_L, KEY_R] 
  key_flags.chip_a = buffer[0];
  i2c_write_blocking(_i2c, KEY_IC_B, buf, 1, true);
  i2c_read_blocking(_i2c, KEY_IC_B, buffer, 1, false); // [S2_U, S2_D, S2_L, S2_R, KEY_A, KEY_B, KEY_X, KEY_Y]
  key_flags.chip_b = buffer[0];
  i2c_write_blocking(_i2c, KEY_IC_C, buf, 1, true);
  i2c_read_blocking(_i2c, KEY_IC_C, buffer, 1, false); // [S2_C, KEY_SE, KEY_SR, KEY_ZR, S1_C, KEY_ST, KEY_SL, KEY_ZL]
